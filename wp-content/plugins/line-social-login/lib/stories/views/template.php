...
<div class="wpmet-dashboard-widget-block">
    <div class="wpmet-title-bar">
        ...
        foreach ($this->plugin_link as $k => $link) {
            echo wp_kses('<a target="_blank" href="' . $link[1] . '">' . $link[0] . '</a>', \WP_Social\Helper\Helper::get_kses_array());
...
foreach ($this->stories as $story) :
    if($story['type'] === 'news' || $story['type'] === '') :
        ?>
        <div class="wpmet-str <?php echo esc_attr(((isset($story['story_image']) && $story['story_image'] != '') ? 'with-image' : '')); ?>">
    ...
    ?>
        <div class="wpmet-str">
            <a target="_blank" href="<?php echo esc_url($story['story_link']) ?>">
                <img class="wpmet-banner" src="<?php echo esc_url(isset($story['story_image']) && $story['story_image'] != '' ? $story['story_image'] : '#'); ?>" />
            </a>
        </div>
        <?php
    endif;
