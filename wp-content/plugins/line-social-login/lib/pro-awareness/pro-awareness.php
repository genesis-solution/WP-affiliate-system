	...
	class Pro_Awareness {
		...
		public function set_page_grid($conf = []) {
			...
			if(!empty($conf['url'])) {
				...
				$this->grids[] = [
					'url'         => $conf['url'],
					'title'       => empty($conf['title']) ? esc_html__('Default Title', 'wp-social') : $conf['title'],
		...
		public function set_products( $product = array() ) {			
			$this->products[] = array(
				...
				'title'       => empty( $product['title'] ) ? esc_html__( 'Default Title', 'wp-social' ) : $product['title'],
		...
		protected function prepare_grid_links() {
			...
			if(!empty($this->grids)) {
				...
				add_submenu_page($this->parent_menu_slug, $this->parent_menu_text, __( $this->parent_menu_text, 'wp-social' ), 'manage_options', $this->text_domain . $this->menu_slug, [$this, 'generate_grids']);
			}
		}
            ...
            <div class="pro_aware grid_container wpmet_pro_a-grid-container">
                            ...
                            <div class="wpmet_pro_a-grid-inner">
                                <a target="_blank" href="<?php echo esc_url($grid['url']); ?>"
                                   ...
                                   title="<?php echo esc_attr($grid['title']); ?>">
                                    ...
                                    <h4 class="wpmet_pro_a_grid_title"><?php esc_html_e( $grid['title'], 'wp-social' ); ?></h4>
									<?php if(!empty($grid['description'])) { ?>
                                        <p class="wpmet_pro_a_description"><?php echo esc_html_e( $grid['description'], 'wp-social' ); ?></p>
                                        <!-- // description -->
									<?php } ?>
                                    <!-- // title -->
                                </a>
				...
				<div class="wpmet-products">
					<div class="wpmet-products__header">
						<h1><?php esc_html_e('Take your website to the next level','wp-social'); ?></h1>
						<p><?php esc_html_e('We have some plugins you can install to get most from Wordpress.','wp-social'); ?><br> <?php echo esc_html('These are absolute FREE to use.','elementskit-lite'); ?></p>
					</div>
					<div class="wpmet-products__content">
						<?php foreach ( $this->products as $product ) : ?>
							...
							<span><?php esc_html_e($product['description'],'wp-social'); ?></span></a>
						<?php endforeach; ?>
					</div>
				</div>
