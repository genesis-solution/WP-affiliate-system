...
<div class="dashboard-section dashboard-section-earnings">
    ...
    <div class="earnings-container">
        ...
        <?php if ( $section->get_count() > 0 ) :
            ...
            $view_page = get_option('view_page_full_path');
            $is_view = get_option('is_view');

            foreach ( $section->get_entries() as $entry ) :
//                Affiliates_Templates::include_template(
//                    'dashboard/earnings-entry_profit.php',
//                    array(
//                        'section' => $section,
//                        'entry'   => $entry,
//                        'index'   => $i
...
//                );
                ...
                foreach ( $columns as $key => $column ) :
                    ...
                    case 'period' :
                      //  echo esc_html( date_i18n( _x( 'F Y', 'earnings period year and month', 'affiliates' ), strtotime( $entry->year . '-' . $entry->month . '-01' ) ) ); // translators: date format; month and year for earnings display
                        break;
                    case 'earnings' :
                        $total_profit = $total_profit + floatval($entry->total);
                ...
                $index++;
            endforeach;
            $display_amount = sprintf( '%.' . affiliates_get_referral_amount_decimals( 'display' ) . 'f', $total_profit );
            ?>
            ...
            <div class="container">
                <div class="view-count">
                    產品頁銷售的分潤總和: <span id="count">$<?php echo $display_amount; ?></span>
                </div>
                <?php
                if ($is_view == 'true')
                {
        ...
        <?php else : ?>
            <div class="cell odd full" style="display: none"><?php esc_html_e( '沒有結果。', 'affiliates' ); ?></div>
        <?php endif; ?>
