...
function affiliates_admin() {
	...
	echo '<h1>' . __('附屬公司概覽', 'affiliates' ) . '</h1>';
	...
	echo '<h2>' . sprintf( __( '%d 天圖表', 'affiliates' ), $days_back ) . '</h2>';
	...
	$accepted_icon = "<img class='icon' alt='" . __('已接受', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/accepted.png'/>";
	$closed_icon = "<img class='icon' alt='" . __('關閉', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/closed.png'/>";
	$pending_icon = "<img class='icon' alt='" . __('待辦的', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/pending.png'/>";
	$rejected_icon = "<img class='icon' alt='" . __('拒絕', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/rejected.png'/>";

	$statuses = array(
		AFFILIATES_REFERRAL_STATUS_ACCEPTED => array( 'name' => __('已接受', 'affiliates' ), 'icon' => $accepted_icon ),
		AFFILIATES_REFERRAL_STATUS_CLOSED => array( 'name' => __('關閉', 'affiliates' ), 'icon' => $closed_icon ),
		AFFILIATES_REFERRAL_STATUS_PENDING => array( 'name' => __('待辦的', 'affiliates' ), 'icon' => $pending_icon ),
		AFFILIATES_REFERRAL_STATUS_REJECTED => array( 'name' => __('拒絕', 'affiliates' ), 'icon' => $rejected_icon ),
	);
	...
	if ( count( $referral_stats ) > 0 ) {
		foreach ( $referral_stats as $currency_id => $stats ) {
			...
			echo '<div class="status heading">' . esc_html__('地位', 'affiliaes' ) . '</div>';
			echo '<div class="count heading">' . esc_html__( 'Count', 'affiliates' ) . '</div>';
			echo '<div class="amount heading">' . esc_html__('數量', 'affiliates' ) . '</div>';
	...
	echo '<div class="hits label">' . esc_html__('點擊數', 'affiliates' ) . '</div>';
	echo '<div class="visits count">' . intval( $visits ) . '</div>';
	echo '<div class="visits label">' . esc_html__('訪問次數', 'affiliates' ) . '</div>';
	echo '</div>';
