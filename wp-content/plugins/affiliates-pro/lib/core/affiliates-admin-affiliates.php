...
function affiliates_admin_affiliates() {
	...
	if ( isset( $_POST['action'] ) ) {
		...
		switch ( $_POST['action'] ) {
			case 'add' :
				...
				$notice_msg = esc_html__('已添加附屬機構。', 'affiliates' );
				break;
			case 'edit' :
				...
				$notice_msg = esc_html__('附屬機構已更新。', 'affiliates' );
				break;
			case 'remove' :
				affiliates_admin_affiliates_remove_submit();
				$notice_msg = esc_html__('附屬機構已刪除。', 'affiliates' );
				break;
			...
			case 'affiliate-action' :
				...
				$notice_msg = esc_html__('批量操作已執行', 'affiliates' );
				break;
		}
		if ( strlen( $notice ) > 0 ) {
	...
	$output .= "<a title='" . esc_html__('點擊添加新會員', 'affiliates' ) . "' class='button add' href='" . esc_url( $current_url ) . "&action=add'><img class='icon' alt='" . esc_html__('添加', 'affiliates') . "' src='". AFFILIATES_PLUGIN_URL ."images/add.png'/><span class='label'>" . esc_html__('新會員', 'affiliates') . "</span></a>";
	$output .= '<div style="float:right">';
	$output .= sprintf( '<div class="button toggle-button %s" id="filters-toggle">', ( $show_filters ? 'on' : 'off' ) );
	$output .= esc_html__('過濾器', 'affiliates' );
	...
	$column_display_names = array(
		...
		'email'        => __('電子郵件', 'affiliates' ),
		'user_login'   => __('用戶名', 'affiliates' ),
		'from_date'    => __('從', 'affiliates' ),
		'thru_date'    => __('直到', 'affiliates' ),
		'status'       => __('地位', 'affiliates' ),
		'edit'         => __('編輯', 'affiliates' ),
		'remove'       => __('消除', 'affiliates' ),
		'links'        => __('鏈接', 'affiliates' ),
	);
	...
	$output .=
			'<label class="description" for="setfilters">' . __('過濾器', 'affiliates' ) . '</label>' .
			'<form id="setfilters" action="" method="post">' .
				...
				'<input class="button" type="submit" name="clear_filters" value="' . __('清除', 'affiliates' ) . '"/>' .
	...
	$output .= '<option selected="selected" value="-1">' . esc_html( __('批量操作', 'affiliates' ) ) . '</option>';
	$output .= '<option value="remove-affiliate">' . esc_html( __('刪除附屬機構', 'affiliates' ) ) . '</option>';
	$output .= '<option value="status-pending">' . esc_html( __('將狀態設置為待處理', 'affiliates' ) ) . '</option>';
	$output .= '<option value="status-active">' . esc_html( __('將狀態設置為活動', 'affiliates' ) ) . '</option>';
	$output .= '</select>';
	$output .= sprintf( '<input class="button" type="submit" name="bulk" value="%s" />', esc_attr( __('申請', 'affiliates' ) ) );
	...
	if ( count( $results ) > 0 ) {
		for ( $i = 0; $i < count( $results ); $i++ ) {
			...
			if ( $is_deleted = ( strcmp( $result->status, 'deleted' ) == 0 ) ) {
				...
				$name_suffix .= " " . esc_html__('已刪除', 'affiliates' );
			...
			if ( $is_inoperative = ! ( ( $result->from_date <= $today ) && ( $result->thru_date == null || $result->thru_date >= $today ) ) ) {
				...
				$name_suffix .= " " . esc_html__('無效', 'affiliates' );
			}
			...
			$output .= "<td class='edit'><a href='" . esc_url( add_query_arg( 'paged', $paged, $current_url ) ) . "&action=edit&affiliate_id=" . $result->affiliate_id . "' alt='" . __('編輯', 'affiliates') . "'><img src='". AFFILIATES_PLUGIN_URL ."images/edit.png'/></a></td>";
			$output .= "<td class='remove'>" .
				( !$is_deleted && ( !isset( $result->type ) || ( $result->type != AFFILIATES_DIRECT_TYPE )  ) ?
				"<a href='" . esc_url( $current_url ) . "&action=remove&affiliate_id=" . $result->affiliate_id . "' alt='" . __('消除', 'affiliates') . "'><img src='". AFFILIATES_PLUGIN_URL ."images/remove.png'/></a>"
				: "" ) .
				"</td>";
					...
					if ( $total ) {
						...
						switch( $status ) {
							case AFFILIATES_REFERRAL_STATUS_CLOSED :
								$output .= sprintf( __('<span style="cursor:help" title="%s">已關閉</span>', 'affiliates' ), esc_attr( __('已關閉推薦的累計總額（已支付的佣金）。', 'affiliates' ) ) );
								break;
							case AFFILIATES_REFERRAL_STATUS_ACCEPTED :
								$output .= sprintf( __('<span style="cursor:help" title="%s">已接受</span>', 'affiliates' ), esc_attr( __('接受推薦的累計總數（未支付佣金）。', 'affiliates' ) ) );
								break;
							case AFFILIATES_REFERRAL_STATUS_PENDING :
								$output .= sprintf( __('<span style="cursor:help" title="%s">待處理</span>', 'affiliates' ), esc_attr( __( 'Accumulated total for pending referrals.', 'affiliates' ) ) );
								break;
		...
		$output .= '<tr><td colspan="' . $num_columns . '">' . __('沒有結果。', 'affiliates' ) . '</td></tr>';
	}
