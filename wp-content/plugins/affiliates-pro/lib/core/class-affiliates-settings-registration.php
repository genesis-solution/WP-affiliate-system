...
class Affiliates_Settings_Registration extends Affiliates_Settings {
	...
	public static function init() {
		...
		self::$default_fields = array(
			...
			'user_login' => array( 'obligatory' => false, 'enabled' => true, 'label' => __( '用戶名', 'affiliates' ), 'required' => true, 'is_default' => true, 'type' => 'text' ),
			'user_email' => array( 'obligatory' => true, 'enabled' => true, 'label' => __( '電子郵件', 'affiliates' ), 'required' => true, 'is_default' => true, 'type' => 'text' ),
			'user_url'	 => array( 'obligatory' => false, 'enabled' => true, 'label' => __( '網站', 'affiliates' ), 'required' => false, 'is_default' => true, 'type' => 'text' ),
			'password'	 => array( 'obligatory' => false, 'enabled' => false, 'label' => __( '密碼', 'affiliates' ), 'required' => false, 'is_default' => true, 'type' => 'password' )
		);
	}
	...
	public static function section() {
					...
					if ( !empty( $field_name[$i] ) ) {
						...
						if ( !empty( $name ) && !isset( $fields[$name] ) ) {
							$fields[$name] = array(
								...
								'is_default' => true, //key_exists( $field_name[$i], self::$default_fields ),
		...
		echo
			'<p>' .
			'<label>' .
			esc_html__('地位', 'affiliates' ) .
			' ' .
			'<select name="affiliate_status">' .
			sprintf( '<option value="active" %s>', $affiliate_status == 'active' ? ' selected="selected" ' : '' ) .
			esc_html__( '積極的', 'affiliates' ) .
			'</option>' .
			sprintf( '<option value="pending" %s>', $affiliate_status == 'pending' ? ' selected="selected" ' : '' ) .
			esc_html__('待辦的', 'affiliates' ) .
			'</option>' .
			'</select>' .
			'</label>';
		echo '<br/>';
		echo '<span class="description">';
		esc_html_e( '這決定了新的聯屬網絡營銷申請是否需要手動批准或是否被自動接受。', 'affiliates' );
		echo ' ';
		echo wp_kses( __( '<em>待處理</em>將需要新聯營公司的手動批准。 <em>Active</em>將自動接受新的聯屬會員。', 'affiliates' ), array( 'em' => array() ) );
		echo '</span>';
		echo '</p>';

		echo '<p>';
		echo '<label for="terms_post_id">';
		esc_html_e( '條款', 'affiliates' );
		echo ' ';
		wp_dropdown_pages(
			array(
				'name'              => 'terms_post_id',
				'echo'              => true,
				'show_option_none'  => __( '&mdash; 選擇 &mdash;' ),
				'option_none_value' => '',
		...
		echo '<br/>';
		echo '<span class="description">';
		esc_html_e( '條款和條件', 'affiliates' );
		echo ' &mdash; ';
		esc_html_e( '如果選擇，免責聲明和頁面鏈接將與註冊表一起顯示。', 'affiliates' );
		echo '</span>';
		echo '</p>';

		// registration fields
		echo '<h3>' . __( '附屬機構登記表', 'affiliates' ) . '</h3>';
		echo '<p class="description">';
		esc_html_e( '聯營公司註冊表上提供了以下字段。', 'affiliates' );
		echo '</p>';
		...
		echo '</th>';
		echo '<th>';
		esc_html_e( '啟用', 'affiliates' );
		echo '</th>';
		echo '<th>';
		esc_html_e( '字段名稱', 'affiliates' );
		echo '</th>';
		echo '<th>';
		esc_html_e( '字段標籤', 'affiliates' );
		echo '</th>';
		echo '<th>';
		esc_html_e( '必需的', 'affiliates' );
		echo '</th>';
		echo '<tr>';
		echo '<th>';
		echo '</tr>';
		...
		foreach( $registration_fields as $name => $field ) {
			...
			echo sprintf( '<button class="field-up button" type="button" value="%d">%s</button>', $i, esc_html( __( '向上', 'affiliates' ) ) );
			echo sprintf( '<button class="field-down button" type="button" value="%d">%s</button>', $i, esc_html( __( '向下', 'affiliates' ) ) );
			if ( !$field['is_default'] ) {
				echo sprintf( '<button class="field-remove button" type="button" value="%d">%s</button>', $i, esc_html( __('消除', 'affiliates' ) ) );
			}
			echo '</td>';
			echo '</tr>';
			$i++;
		...
		echo '<p>';
		echo sprintf( '<button class="field-add button" type="button" value="%d">%s</button>', $i, esc_html( __( '添加字段', 'affiliates' ) ) );
		echo '</p>';
