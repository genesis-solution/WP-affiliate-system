		...
		switch ( $action ) {
			...
			case 'confirm' :
				...
				if ( is_array( $robot_hits ) && count( $robot_hits ) > 0 ) {
					...
					echo '<thead>';
					echo '<tr>';
					echo '<th style="padding: 4px; border-bottom: 1px solid #999;">' . esc_html__('點擊數', 'affiliates' ) . '</th>';
					echo '<th style="padding: 4px; border-bottom: 1px solid #999;">' . esc_html__('用戶代理', 'affiliates' ) . '</th>';
					echo '</tr>';
					echo '</thead>';
					echo '<tbody>';
					$count = 0;
					...
					echo esc_html__('全部的', 'affiliates' );
					echo '</td>';
					echo '</tr>';
					echo '</tbody>';
					echo '</table>';
