...
class Affiliates_Totals {
	...
	public static function view() {
		...
		$column_display_names = array(
			...
			'total'        => __('全部的', 'affiliates' ),
			'currency_id'  => __('貨幣', 'affiliates' )
		);
		...
		$output .=
			...
			"<span class='label'>" . __('密切推薦', 'affiliates') . "</span>" .
			"</a>";
		$output .= "</p>";
		$output .= '</div>';
		...
		$affiliate_status_descriptions = array(
			...
			AFFILIATES_AFFILIATE_STATUS_PENDING   => __('待辦的', 'affiliates' ),
		...
		$currency_select .= __('貨幣', 'affiliates' );
		$currency_select .= ' ';
	...
	public static function render_results( $results ) {
		$output = "";
		$column_display_names = array(
			'datetime'    => __('日期', 'affiliates' ),
			'post_title'  => __('郵政', 'affiliates' ),
			'name'        => __('附屬機構', 'affiliates' ),
			'amount'      => __('數量', 'affiliates' ),
			'currency_id' => __('貨幣', 'affiliates' ),
		...
		if ( count( $results ) > 0 ) {
			...
			$output .= '<tr><td colspan="' . count( $column_display_names ) . '">' . __('沒有結果。', 'affiliates' ) . '</td></tr>';
		}
		$output .= '</tbody>';
		$output .= '</table>';
		return $output;
