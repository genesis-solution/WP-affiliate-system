...
class Affiliates_Contact extends WP_Widget {
	...
	public static function render_form( $widget_id = '', $amount = null, $currency_id = null ) {
		...
		if ( !$send ) {
			...
			echo '<div>';
			echo '<label ' . $sender_class . ' id="affiliates-contact-form' . $ext . '-sender-label" for="sender">' . __('姓名', 'affiliates' ) . '</label>';
			echo '<input id="affiliates-contact-form' . $ext . '-sender" name="sender" type="text" value="' . esc_attr( $sender ) . '"/>';
			echo '<label ' . $email_class . ' id="affiliates-contact-form' . $ext . '-email-label" for="email">' . __('電子郵件', 'affiliates' ) . '</label>';
