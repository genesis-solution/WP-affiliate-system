...
global $affiliates_options, $affiliates_version, $affiliates_admin_messages, $selected_affiliate_id;
...
function my_custom_middleware() {
    ...
    $request_url = $_SERVER['REQUEST_URI']; // http://localhost/shareing/product/short-sleeve-t-shirt/
    $path = parse_url($request_url, PHP_URL_PATH);
    $segments = explode('/', rtrim($path, '/'));
    $productName = end($segments);
    $productPath = prev($segments);
//    echo $productPath; // Output: product
//    echo $productName; // Output: product-name
    if ($productPath == 'product') {
        ...
        if (isset($product) && !empty($product))
        {
            ...
            $shared_id = $wpdb->get_row($wpdb->prepare(
                "SELECT `affiliate_id`, `user_id` FROM {$table_name} WHERE `post_id` = %s AND `status` = %s;",
                $product->ID, 'accept'
            ));

            if (isset($shared_id) && !empty($shared_id))
            {
                $pname = get_option( 'aff_pname', AFFILIATES_PNAME );

                $affiliates = affiliates_get_user_affiliate($shared_id->user_id);

                if(isset($pname) && !empty($pname) && isset($affiliates))
                {
                    $affiliate_id = $affiliates[0];
                    $current_url  =add_query_arg($pname, $affiliate_id, ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);

                    session_start();
                    $saved_session = isset($_SESSION['selected_affliate_id']) ? $_SESSION['selected_affliate_id'] : "";
                    if ($saved_session != $affiliate_id)
                    {
                        $_SESSION['selected_affliate_id'] = $affiliate_id;
                    ...
                    session_start();
                    $_SESSION["selected_affliate_id"] = "";
                    unset($_SESSION["selected_affliate_id"]);
        ...
        else
        {
            ...
            $saved_session = isset($_SESSION['selected_affliate_id']) ? $_SESSION['selected_affliate_id'] : "";
            $pname = get_option( 'aff_pname', AFFILIATES_PNAME );
            if (!empty($saved_session) && !empty($pname))
            {
                $current_url  =add_query_arg($pname, $saved_session, ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
                wp_redirect($current_url);
                exit;
            }
        }
...
function affiliates_parse_request( &$wp ) {
	...
	if ( $affiliate_id ) {
		...
		$days =  100;//apply_filters( 'affiliates_cookie_timeout_days', get_option( 'aff_cookie_timeout_days', AFFILIATES_COOKIE_TIMEOUT_DAYS ), $affiliate_id );
		if ( $days > 0 ) {
