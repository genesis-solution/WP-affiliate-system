...
class Affiliates_Registration {
	...
	public static function render_form( $options = array() ) {
		...
		if ( affiliates_user_is_affiliate_status( null, 'pending' ) ) {
			...
			$output .= '<p>';
			$output .= __( '您的聯屬網絡營銷申請正在等待批准。', 'affiliates' ); // Your affiliate application is pending approval.
			$output .= '</p>';
			$output .= '</div>';
			return $output;
		}
		...
		if ( affiliates_user_is_affiliate_status( null, 'deleted' ) ) {
			$output .= '<div class="affiliates-registration registered pending">';
			$output .= '<p>';
			$output .= __( '您的聯屬帳戶已關閉。', 'affiliates' ); // Your affiliate account is closed.
			$output .= '</p>';
			$output .= '</div>';
			return $output;
		}
		...
		if ( $is_affiliate = affiliates_user_is_affiliate() ) {
			...
			$output .= '<p>';
			$output .= __( '您已經註冊為會員。', 'affiliates' );// You are already registered as an affiliate.
			$output .= '</p>';
