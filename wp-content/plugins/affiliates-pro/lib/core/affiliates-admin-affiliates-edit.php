...
function affiliates_admin_affiliates_edit( $affiliate_id ) {
	...
	if ( empty( $affiliate ) ) {
		wp_die( esc_html__('沒有這樣的附屬機構。', 'affiliates' ) );
	}
		...
		foreach ( $_POST['errors'] as $error ) {
			switch ( $error ) {
				case AFFILIATES_ADMIN_AFFILIATES_ERROR_NAME_EMPTY :
					$notice_msg[] = __('名稱不能為空。', 'affiliates' );
					break;
				case AFFILIATES_ADMIN_AFFILIATES_ERROR_USERNAME :
					$notice_msg[] = __('用戶名不存在。', 'affiliates' );
					break;
				default:
					$notice_msg[] = __('出了些問題。', 'affiliates' );
					break;
			}
		}
	...
	$output .=
		...
		'<option value="pending" ' . ( $status == 'pending' ? 'selected="selected"' : '' ) . ' >' . __('待辦的', 'affiliates' ) . '</option>' .
	...
	$output .=
		...
		'<input class="button button-primary" type="submit" value="' . __('節省', 'affiliates' ) . '"/>' .
		'<input type="hidden" value="edit" name="action"/>' .
		' ' .
		'<a class="cancel button" href="' . esc_url( $current_url ) . '">' . __('取消', 'affiliates' ) . '</a>' .
		'</div>' .
		'</div>' . // .affiliate.edit
		'</form>' .
