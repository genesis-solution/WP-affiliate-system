...
function affiliates_admin_hits_affiliate() {
	...
	$status_icons = array(
		AFFILIATES_REFERRAL_STATUS_ACCEPTED => "<img class='icon' alt='" . __('已接受', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/accepted.png'/>",
		AFFILIATES_REFERRAL_STATUS_CLOSED   => "<img class='icon' alt='" . __('關閉', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/closed.png'/>",
		AFFILIATES_REFERRAL_STATUS_PENDING  => "<img class='icon' alt='" . __('待辦的', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/pending.png'/>",
		AFFILIATES_REFERRAL_STATUS_REJECTED => "<img class='icon' alt='" . __('拒絕', 'affiliates') . "' src='" . AFFILIATES_PLUGIN_URL . "images/rejected.png'/>",
	);
	$status_checkboxes = '';
	...
	$output .=
		'<div class="filters">' .
			...
			'<form id="setfilters" action="" method="post">' .
				...
				'<div class="filter-section">' .
				'<span style="padding-right:1em">' . __('地位', 'affiliates' ) . '</span>' .
				' ' .
				$status_checkboxes .
				'</div>' .
	...
	if ( count( $results ) > 0 ) {
			...
			if ( $expanded_referrals || $expanded_hits ) {
				...
				if ( $expanded_referrals ) {
					...
					if ( count($referrals) > 0 ) {
						...
						$output .= '<thead>';
						$output .= '<tr>';
						$output .= '<th scope="col" class="datetime">' . __('日期', 'affiliates' ) . '</th>';
						$output .= '<th scope="col" class="post-id">' . __('郵政', 'affiliates' ) . '</th>';
						$output .= '</tr>';
						$output .= '</thead>';
						$output .= '<tbody>';
				...
				if ( $expanded_hits ) {
					...
					if ( count( $hits ) > 0 ) {
						...
						$output .= '<p class="description">' . __('點擊數', 'affiliates' ) . sprintf( ' (%d/%d)', count( $hits ), $hits_count ) . '</p>';
						...
						$output .= '<th scope="col" class="affiliate-id">' . __('附屬機構', 'affiliates' ) . '</th>';
						$output .= '<th scrope="col" class="src-uri">' . __('源 URI', 'affiliates' ) . '</th>';
						$output .= '<th scrope="col" class="src-uri">' . __('登陸 URI', 'affiliates' ) . '</th>';
						$output .= '<th scope="col" class="hit-user-agent">' . __('用戶代理', 'affiliates' ) . '</th>';
						$output .= '</tr>';
						$output .= '</thead>';
						$output .= '<tbody>';
						foreach ( $hits as $hit ) {
		...
		$output .= '<tr><td colspan="5">' . __('沒有結果。', 'affiliates' ) . '</td></tr>';
	}
