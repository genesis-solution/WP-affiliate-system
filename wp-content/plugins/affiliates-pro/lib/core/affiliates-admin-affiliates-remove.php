...
function affiliates_admin_affiliates_remove( $affiliate_id ) {
	...
	if ( empty( $affiliate ) ) {
		wp_die( __('沒有這樣的附屬機構。', 'affiliates' ) );
	}
	...
	$output =
		...
		'<input class="button button-primary" type="submit" value="' . esc_html__('消除', 'affiliates' ) . '"/>' .
...
function affiliates_admin_affiliates_bulk_remove() {
	...
	$output .= wp_nonce_field( 'admin', AFFILIATES_ADMIN_AFFILIATES_ACTION_NONCE, true, false ) .
		'<input class="button button-primary" type="submit" name="bulk" value="' . esc_html__('消除', 'affiliates' ) . '"/>' .
		' ' .
		'<a class="cancel button" href="' . esc_url( $current_url ) . '">' . esc_html__('取消', 'affiliates' ) . '</a>' .
		'</div>' .
		'</div>' . // .affiliate.remove
