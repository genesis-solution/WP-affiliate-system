...
function affiliates_admin_hits_uri() {
	...
	$column_display_names = array(
		...
		'src_uri'    => __('源 URI', 'affiliates' ),
		'dest_uri'   => __('登陸 URI', 'affiliates' ),
	...
	$output .=
		'<div class="filters">' .
			...
			'<form id="setfilters" action="" method="post">' .
				...
				'</div>' .
				'<div class="filter-section">' .
				'<span style="padding-right:1em">' . __('推薦狀態', 'affiliates' ) . '</span>' .
				' ' .
				$status_checkboxes .
				' ' .
				...
				if ( !empty( $result->campaign_id ) ) {
					if ( $campaign = Affiliates_Campaign::get_affiliate_campaign( $result->affiliate_id, $result->campaign_id ) ) {
						$output .= sprintf(
							...
							esc_html( stripslashes( $campaign->name ) ),
