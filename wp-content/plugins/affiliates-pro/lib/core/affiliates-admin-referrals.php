...
// 	}
	...
	$status_select .= __('地位', 'affiliates' );
	$status_select .= ' ';
...
// 			$output .= "</form>";
			...
			$output .= sprintf( '<img src="%s" alt="%s"/>', AFFILIATES_PLUGIN_URL . 'images/edit.png', __('編輯', 'affiliates' ) );
			$output .= '</a>';
			$output .= '</td>';
			...
			$output .= sprintf( '<img src="%s" alt="%s"/>', AFFILIATES_PLUGIN_URL . 'images/remove.png', __('消除', 'affiliates' ) );
			$output .= '</a>';
			$output .= '</td>';
		...
		$output .= '<tr><td colspan="' . $column_count . '">' . __('沒有結果。', 'affiliates' ) . '</td></tr>';
	}
