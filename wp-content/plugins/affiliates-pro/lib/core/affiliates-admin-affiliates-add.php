...
function affiliates_admin_affiliates_add() {
		...
		foreach ( $_POST['errors'] as $error ) {
			switch ( $error ) {
				case AFFILIATES_ADMIN_AFFILIATES_ERROR_NAME_EMPTY :
					$notice_msg[] = esc_html__('名稱不能為空。', 'affiliates' );
					break;
				case AFFILIATES_ADMIN_AFFILIATES_ERROR_USERNAME :
					$notice_msg[] = esc_html__('用戶名不存在。', 'affiliates' );
					break;
				default :
					$notice_msg[] = esc_html__('出了些問題。', 'affiliates' );
					break;
			}
		}
	...
	$output =
		...
		'<input class="button button-primary" type="submit" value="' . esc_html__('添加', 'affiliates' ) . '"/>' .
		'<input type="hidden" value="add" name="action"/>' .
		' ' .
		'<a class="cancel button" href="' . esc_url( $current_url ) . '">' . esc_html__('取消', 'affiliates' ) . '</a>' .
		'</div>' .
